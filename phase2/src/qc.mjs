...
export async function inspectMp4(file) {
  ...
  return { pass: Boolean(video && audio && video.codec_name === 'h264' && audio.codec_name === 'aac' && video.width === 1080 && video.height === 1920 && duration >= 20 && duration <= 45 && fps > 29 && fps < 31), duration, video: video && { codec: video.codec_name, width: video.width, height: video.height, fps }, audio: audio?.codec_name, bytes: fileStat.size };
}
