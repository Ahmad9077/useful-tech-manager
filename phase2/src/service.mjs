...
export class Phase2Service {
  ...
  async runStage(job, payload) {
    ...
    else if (stage === 'QC') { const revision = this.store.getRevision(contentId, revisionNumber); const qc = await inspectMp4(revision.artifact_path); if (!qc.pass) throw new Error('RENDER_QC_FAILED'); const contentState = this.store.getContent(contentId).state; if (contentState === 'PRODUCING') this.store.transition(contentId, 'QC', 'pipeline'); if (this.store.getContent(contentId).state === 'QC') this.store.transition(contentId, 'READY_FOR_REVIEW', 'pipeline'); await this.sendReady(this.store.getContent(contentId), revision, qc.duration); this.store.updateProgress({ contentId, stage: 'READY_FOR_REVIEW', workerStatus: 'WAITING', jobId: null, waitingReason: 'WAITING_FOR_USER_APPROVAL', waitingSince: isoNow(), lastSuccessfulStep: 'QC passed and MP4 delivered' }); this.store.audit('READY_FOR_REVIEW_DELIVERED', 'pipeline', { contentId, revisionNumber }); return null; }
