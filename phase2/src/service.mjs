...
export class Phase2Service {
  ...
  async pollTelegram() {
    ...
    for (const update of updates) { try { const result = this.control.handleUpdate(update); const callback = update.callback_query; if (callback) await this.telegram.answerCallback(callback.id, result.ignored ? 'Not authorized' : 'Saved'); const message = callback?.message || update.message; if (!result.ignored && message?.chat?.id === Number(this.config.telegram.ownerChatId)) { let text; if (result.type === 'approved') { const archivePath = await archiveApproved({ store: this.store, contentId: result.approval.content_id, revisionNumber: Number(result.approval.revision_number), archiveRoot: this.config.archiveDir, approvedWorkRoot: path.join(this.config.dataDir, 'work') }); this.store.queueJob('PUBLISH_APPROVED', { contentId: result.approval.content_id, revisionNumber: Number(result.approval.revision_number), environment: 'sandbox' }, isoNow(), `publish:${result.approval.content_id}:${result.approval.revision_number}:sandbox`); text = `تم اعتماد المراجعة وحفظ النسخة النهائية في الأرشيف: ${path.basename(archivePath)}`; } else text = result.type === 'revision-requested' ? 'تم تسجيل طلب التعديل وإنشاء مراجعة جديدة.' : result.type === 'rejected' ? 'تم رفض الفيديو ولن يتم نشره.' : result.type === 'skipped' ? 'تم تخطي اليوم ولن يتم إنشاء بديل تلقائيًا.' : result.text || (result.type === 'status' ? `الحالة: ${result.items.map((item) => `${item.topic} — ${item.state}`).join('\n') || 'لا يوجد محتوى بعد'}` : 'تم استلام طلبك.'); await this.telegram.call('sendMessage', { chat_id: message.chat.id, text }); } } catch (error) { this.store.audit('TELEGRAM_UPDATE_FAILED', 'service', { error: redact(error.message).slice(0, 160) }); }
    }
  }
  async flushOutbox() {
