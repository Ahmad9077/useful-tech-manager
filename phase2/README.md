...
Telegram messages are interpreted by a protected Gemini semantic layer using only bounded conversation context. Model output is a closed JSON proposal, never executable instructions: owner authorization, SQLite workflow state, exact revision fingerprints, and TikTok environment checks remain deterministic. The only offline parsing retained is the exact safety fallback for `/status` and `/cancel`.
