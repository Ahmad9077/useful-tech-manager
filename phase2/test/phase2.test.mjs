...
function semanticControl(store, handler) { return new TelegramControl({ store, config, signingSecret: 'semantic-test-secret', interpreter: new FixedSemanticInterpreter(handler) }); }
...
test('semantic schema is closed and rejects malformed model output', () => {
  assert.equal(validateInterpretation(semantic('STATUS')).intent, 'STATUS');
  assert.throws(() => validateInterpretation({ ...semantic('STATUS'), extra: true }), /Unexpected/);
  assert.throws(() => validateInterpretation({ ...semantic('STATUS'), confidence: 4 }), /confidence/);
...
test('semantic conversation resolves idea references, keeps praise non-publishing, and applies explicit exact approval', async (t) => {
  const asset = await fixture(); t.after(() => rm(asset.dir, { recursive: true, force: true })); const store = new Store(); const contentId = ready(store, asset);
  ...
  const selected = await control.handleUpdate(ownerMessage(11, 'الثاني حلو سوّه')); assert.equal(selected.type, 'content-started');
