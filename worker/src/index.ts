...
async function publishVideo(request: Request, env: Env): Promise<Response> {
  ...
  if (mode !== "direct" && mode !== "inbox") throw new AppError("Select a TikTok delivery type.", 422);
  if (mode === "direct" && (String(form.get("music_confirmation") || "") !== "confirmed" || String(form.get("declaration") || "") !== "confirmed")) throw new AppError("Confirm the TikTok declarations before publishing.", 422);
  if (mode === "direct" && commercialContent && !yourBrand && !brandedContent) throw new AppError("Select Your Brand or Branded Content when the commercial disclosure is enabled.", 422);
  if (mode === "direct" && !commercialContent && (yourBrand || brandedContent)) throw new AppError("Enable the commercial disclosure before selecting a commercial content option.", 422);
  if (mode === "direct" && (!Number.isFinite(durationSeconds) || durationSeconds <= 0)) throw new AppError("TikTok needs the selected video's duration before publishing.", 422);
  const token = await accessToken(env, active.open_id);
  ...
  if (mode === "direct") {
    ...
    creator = creatorData((creatorResponse.data || {}) as Record<string, unknown>);
  ...
  await env.DB.prepare("INSERT INTO content_posts (id, open_id, publish_id, post_mode, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)")
    .bind(id, active.open_id, publishId, mode, "INITIALIZED", now(), now()).run();
  try {
  ...
  return json({ id, status: "PROCESSING", delivery: mode === "direct" ? "DIRECT_POST" : "INBOX_DRAFT" });
}
...
async function refreshPostStatus(env: Env, openId: string, id: string, saved: { publish_id: string; post_mode: "direct" | "inbox"; status: string }): Promise<Record<string, unknown>> {
  ...
  return { status, delivery: saved.post_mode === "direct" ? "DIRECT_POST" : "INBOX_DRAFT", fail_reason: data.fail_reason || null, publically_available_post_id: data.publically_available_post_id || null };
}
